...
def flight_generator(fDat, deltaAlt, angle, finalSpeed, getFields, ForeverMode):
	angle = abs(angle)
	if deltaAlt < 0:
		angle =- angle
		direction = -1
...
1.7340981651693024]
	...
	sentCont = False
	while True:
		if (finalAltitude - fDat.altitude) * direction < 3. and abs(fDat.kias - finalSpeed) < 5. and abs(fDat.pitch) < 1.:
			if ForeverMode:
				if not sentCont:
					yield 'CONT'
					sentCont = True
		...
		vel_ned = getVelocityNED()
		(vel_t_ned, targetAngle) = getTargetVelocityNED(leveling = True)
		vel_diff = vel_t_ned - vel_ned
		accNED = vel_diff / ACC_FACTOR

		def pitchSteps(pitch):
			for x in range(-24, 24):
				yield float(x) / 30. + pitch
		...
		for p in pitchSteps(0.):
			...
			if diff < bestDeltaAcc:
				bestDeltaAcc = diff
				bestPitch = p
		print("acc_t: {}, cur speed: {:.5}, tgt speed: {:.5}, pitch_t: {:.4}, height: {:.4}, height_t: {:.4}".format(
			accNED, float(fDat.kias), float(finalSpeed), bestPitch, fDat.altitude, finalAltitude))
...
class AC:
	...
	def PLAN(self, fDat, deltaAlt, angle, finalSpeed, ForeverMode=False):
		self.flightState = flight_generator(fDat, deltaAlt, angle, finalSpeed, lambda: self.getFields(), ForeverMode)
